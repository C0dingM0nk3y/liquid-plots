...
if(run_api){
  ...
  # TEMP WORKAROUND
          ...
          liq.df %<>% subset(grepl("BTC|ETH|SOL|DOGE|GMX|BNB", liq.df$poolName))  #TEMP
...
```{r cleanup1}
var_all <- ls()
var_keep <- c(grep(var_all, pattern = ".df|dir|DF|run|API|key", value=T),
...
for (id in poolId_list){
  source("script_plotPool_v0.R") # makes calc
  
  cat(sprintf("#### %s <br>\n", poolName))
  cat("<div>\n") #needed to encapsulate plot element so not to distrupt the TAB function
  plot1 <- LiqPlots_Trends() #plot1
  print(plot1)
  plot2 <- LiqPlots_ILChanges() #plot2
  print(plot2)
  cat("</div>\n")
...
# Arrange plots (pathwork)
  ...
  pw[[1]] = pw[[1]] + theme(axis.title.x = element_blank()) # Remove x.axis from first subplot
  #pw[[2]] = pw[[2]] + theme(plot.title = element_blank()) # Remove title from second subplot
  
  pw <- pw + plot_layout(heights = c(6, 3), nrow = 2)
