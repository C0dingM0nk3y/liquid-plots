...
subset_list <- c("BTC|ETH|STG|RUNE|DOGE|STG|VET|GMX|MATIC|ADA|OP|ARB") #use REGEX to filter pools by their names 
...
keys.path <- "D:/Clouds/Dropbox/Everywhere/PROJECTS/PiggyBank/Binance/liquid-plot_myCredential.txt" #default is "credential.txt", change if desired
...
### Plots.byPool{.tabset}
Click on tab below to select the group to visualize

**Color codes** \

+ <span style="background-color: orange;">Orange</span>: IL \> Accumulated Earning (_but below Stop-loss_) \
...
#LISTS
list_main <- active.DF[grep(active.DF$poolName, pattern = "BTC|ETH|OP|ARB|GMX"), ]$poolId 
list_examples <- active.DF[grep(active.DF$poolName, pattern = "STG|RUNE|VET|ADA|MATIC|DOGE"), ]$poolId 
list_takeProfit <- active.DF[grep(active.DF$poolName, pattern = "OP"), ]$poolId 
list_stoploss <- active.DF[grep(active.DF$poolName, pattern = "MATIC"), ]$poolId 
list_rest <- poolId_list_original %>%
  setdiff(list_main) %>%
  setdiff(list_examples) %>%
  setdiff(list_takeProfit) %>%
  setdiff(list_stoploss)
```
...
for (id in list_main){
  source("_SCRIPTS/script_plotPool_v1.R") # all plot calc
  }
```

#### Group 2: User Warnings
A few pools with current ratio outside of boundaries. <br>
Here I used the trigger to change the color of the background plot. <br>
An automated transaction can be triggered instead. _[requires POST rights on Binance API]_ 

##### {.tabset}

```{r plots2, results='asis', fig.align='center', fig.height=6, fig.width=10, warning=FALSE}
setStopLoss <- 0.005 #set desired stop loss for plots
takeProfit <- 0

allow_refData = FALSE

for (id in list_examples){
  source("_SCRIPTS/script_plotPool_v1.R") # all plot calc
  }
```


#### Group 3: Take-Profit param {.tabset}
A few examples on how imbalanced pools can also be used as a way to swap a coin for another one, that the investor prefers to accumulate. \
From Impermanent Loss to Impermanent Gain. This also allow to set automated exit thresholds.
...
for (tp in c(0,-0.1, 0.1, 0.2)){
  takeProfit <- tp #set desired stop loss for plots
  
  cat(sprintf("##### take profit: %s%% <br>\n", round(tp*100,0)))
