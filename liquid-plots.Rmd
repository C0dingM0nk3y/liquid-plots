...
source("_SCRIPTS/functions_data.R")
source("_SCRIPTS/functions_plots.R")
```
...
if(run_api){
  ...
  if(fullDataset==FALSE){
    ...
    liq.df %<>% subset(grepl("BTC|ETH|THETA|STG|VET|MATIC|ONE|LINK|ADA|ALGO|ID|RUNE", liq.df$poolName))  #TEMP
  }
...
```{r cleanup1}
var_all <- ls()
var_keep <- c(grep(var_all, pattern = ".df|dir|DF|run|API|key|full", value=T),
...
##### Group 3{.tabset}
Pools that are just there for research purpose
Plotted 2x: second time with 1% stop loss (instead than 0.5%)
